...
class BatteryTariffPeriodCost:
    def __init__(self, buy, sell):
        """ Creates a BatteryTariffPeriodCost object representing the costs of a
        tariff period
...
class BatteryTariffPeriod:
    def __init__(self, cost, start, end):
        """ Creates a BatteryTariffPeriod object representing a time period of a 
        tariff
        cost: A BatteryTariffPeriodCost object representing the cost for this 
        time period
        start: A datetimie.time object representing the start time of the period
        end: A datetimie.time object representing the end time of the period
        """
        self.cost  = cost
        self.start = start
        self.end   = end
...
class Battery(Product):
    ...
    def set_tariff(self, tariff_data):
        ...
        tariff_content = {"tariff_content": tariff_data}
        return self.command('TIME_OF_USE_SETTINGS', tou_settings=tariff_content)

    @staticmethod
    def create_tariff(default_price, periods, provider, plan):
        """ Creates a correctly formatted dictionary of tariff data 
        default_price: A BatteryTariffPeriodCost object representing the price of
        the background time period
        ...
        for period in periods:
            ...
            for (index, bgPeriod) in enumerate(background_time):
                if bgPeriod[0] <= period.start and period.end <= bgPeriod[1]:
                    slotFound = True
                    # If the period matches the start/end times, then we just 
                    # need to adjust the existing background time slot. Otherwise
                    # we need to split it.
                    if bgPeriod[0] == period.start:
                        background_time[index][0] = period.end
                    elif bgPeriod[1] == period.end:     
                        background_time[index][1] = period.start
                    else:
                        background_time.append([period.end, bgPeriod[1]])
                        background_time[index][1] = period.start
            if not slotFound:
                error = True
                break
        ...
        period_names_full    = ["ON_PEAK",  "PARTIAL_PEAK", 
                                "OFF_PEAK", "SUPER_OFF_PEAK"]
        period_names_partial = ["ON_PEAK",  "OFF_PEAK"]
        
        if error or len(costs) > len(period_names_full):
            tariff_Data = none
        else:
            tou_periods     = {}
            buy_price_info  = {}
            ...
            for (index, cost) in enumerate(reversed(sortedCosts)):
                ...
                for period in costs[cost]:
                    # map the second before midnight back to midnight. We 
                    # couldn't do this before as it would have screwed up the 
                    # time comparisons. This is required to get the json in the 
                    # right format
                    if period.end == midnight_End_Time:
                        period.end = midnight_start_time
